...
2. **Skill 注入**：
   - 精确匹配：命中 skill 时注入完整步骤、陷阱、验证方法
   - 模糊匹配：无精确命中时，最多显示 3 个相关 skill 名称和触发条件（避免上下文膨胀）
3. **对话历史**：过滤掉 tool 角色，只保留 user/assistant，限制最近 20 条
...
4. **Skill 注入精确 + 模糊两层**：精确匹配保证专业 skill 不被稀释，模糊匹配在无精确命中时提供相关参考
