...
## TUI展示
![Chromatopsia Terminal](.github/assets/tui.png)
...
- 没有现有配置时，运行 `npx chroma` 会自动进入 onboarding
...
```bash
npx chroma
npx chroma config path
npx chroma doctor
```

## 配置说明

- `npx chroma config path`：查看当前配置文件路径
- `npx chroma doctor`：检查本地运行环境
